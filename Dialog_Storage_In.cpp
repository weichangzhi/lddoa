...
BOOL Dialog_Storage_In::GetConfig()
{
	CString strpathini="";
	...
	int indexid = 0;
	indexid = ::GetPrivateProfileInt("StorageID","InID",0,strpathini);
	indexid++;
	m_StorageInID.Format("JH-%04d-%02d-%02d-%04d",m_timeCurrent.GetYear(),m_timeCurrent.GetMonth(),m_timeCurrent.GetDay(),indexid);
	CString strTemp;
	strTemp.Format("%d",indexid);
	::WritePrivateProfileString("StorageID","InID",strTemp,strpathini);
	...
	nCount = ::GetPrivateProfileInt("Provider","Count",1,strpathini);
	for(i=0;i<nCount;i++)
	{
		strTemp.Format("%d",i+1);
		strTemp="Provider"+strTemp;
		::GetPrivateProfileString("Provider",strTemp,"", strtemp1.GetBuffer(MAX_PATH),MAX_PATH,strpathini);
...
BOOL Dialog_Storage_In::SetConfig()
{
	CString strpathini="";
	::GetCurrentDirectory(_MAX_PATH,strpathini.GetBuffer(_MAX_PATH));
	strpathini.ReleaseBuffer();
	...
	if(cursel==-1 && (!m_strProvider.IsEmpty()))
	{
		...
		if(i==nCount)
		{
			strTemp1.Format("Provider%d",nCount+1);
			::WritePrivateProfileString("Provider",strTemp1,m_strProvider,strpathini);
			strTemp.Format("%d",nCount+1);
			::WritePrivateProfileString("Provider","Count",strTemp,strpathini);
		}
	}
...
/*
   CFont* ptf=m_static1.GetFont(); // 得到原来的字体
   LOGFONT lf; 
   ptf->GetLogFont(&lf); 
   ...
   m_static1.SetFont(&m_editFont); // 设置新字体
*/
	UpdateData(FALSE);
...
void Dialog_Storage_In::OnExcel() 
{
	...
	CString filename ;
	filename.Format("进货单-%s.xls",m_StorageInID);
	Range m_ExlRge; 
...
//	file.Close();
	...
	m_ExlRge.AttachDispatch(m_ExlSheet.GetRange(_variant_t("A2"),_variant_t("J2")),TRUE); 
    m_ExlRge.Merge(_variant_t((long)0));
	m_ExlRge.AttachDispatch(m_ExlSheet.GetRange(_variant_t("A3"),_variant_t("J3")),TRUE); 
    m_ExlRge.Merge(_variant_t((long)0));
	m_ExlRge.AttachDispatch(m_ExlSheet.GetRange(_variant_t("A4"),_variant_t("J4")),TRUE); 
    m_ExlRge.Merge(_variant_t((long)0));
	m_ExlRge.AttachDispatch(m_ExlSheet.GetRange(_variant_t("A5"),_variant_t("J5")),TRUE); 
    m_ExlRge.Merge(_variant_t((long)0));
	...
	strtmp1.Format("    供应商:%s    经办人:%s    部门:%s    付款日期:%s",m_strProvider,m_strOperator,m_strDepartment,strpayment);
	m_ExlRge.SetItem(_variant_t((long)(4)),_variant_t((long)(1)),_variant_t(strtmp1));
	strtmp1.Format("    收货仓库:%s    摘要:%s",m_strStorage,m_Digest);
	...
	m_ExlRge.SetItem(_variant_t((long)(6)),_variant_t((long)(10)),_variant_t("行摘要"));
	...
	m_ExlRge.AttachDispatch(m_ExlSheet.GetRange(_variant_t("A6"), _variant_t("J3")));//获取表单中的列"P"对象
	...
	m_ExlRge.AttachDispatch(m_ExlSheet.GetRange(_variant_t("A3"),_variant_t("J3")));
	m_ExlRge.SetHorizontalAlignment(_variant_t((long)-4152));
	ft.AttachDispatch(m_ExlRge.GetFont()); 
	//ft.SetBold(_variant_t((long)0));//粗体 
...
void Dialog_Storage_In::ResetStorageIn()
{
	m_Digest = "";
	CTime time1 = CTime::GetCurrentTime();
	m_timeCurrent = time1;
...
void Dialog_Storage_In::SetPreviewDlg(Dialog_Storage_Print *PreviewDlg)
{
	UpdateData();
	CString	strCurrent;
	CString	strPayment;
...
void Dialog_Storage_In::OnButtonPrint() 
{
	...
	if (m_printdlg.DoModal()==IDOK)
	{
		...
		writelog(log1);
		PreviewDlg.DrawReport(m_rect,&dc1,TRUE,TRUE);
	}		
}
	...
	if(mysql_real_connect(&myCont,g_MysqlConnect.host,g_MysqlConnect.user,g_MysqlConnect.pswd,g_MysqlConnect.table,g_MysqlConnect.port,NULL,0))
	{
		...
		if(result)
		{
			...
			if(num>=1)//修改记录
			{
				...
				/*
				dlgpro->setpos(700);
				sql.Format("update storage_in_baseinfo set time_make=\"%s\",provider=\"%s\", operator=\"%s\", department=\"%s\", time_payment=\"%s\",storage=\"%s\",digest=\"%s\" where storage_in_id=\"%s\" ",
					strtimecurrent,m_strProvider,m_strOperator,m_strDepartment,strtimepayment,m_strStorage,m_Digest,m_StorageInID);
				if(mysql_query(&myCont,sql)!= 0)
				{
					const char *error = mysql_error(&myCont);
					CString str;
				...
				for(i=0;i<count;i++)
				{
					...
					//storage_name
					if(storage_id.IsEmpty() && storage_name.IsEmpty())
						continue;
					sql.Format("select storagename from storage_name where storageid=\"%s\" ",storage_id);
					if(mysql_query(&myCont,sql)!= 0)
					{
						const char *error = mysql_error(&myCont);
						CString str;
					...
					if(result)
					{
						...
						if(num>=1)
						{
							...
							oldmoney = oldnumber* (item_price);
							dlgpro->setpos(900);
							sql.Format("update storage_name set storagename=\"%s\" where storageid=\"%s\" ",storage_name,storage_id);
							int ret = mysql_query(&myCont,sql);
							if(ret!= 0)
							{
								const char *error = mysql_error(&myCont);
						...
						{//新增记录
							dlgpro->setpos(900);
							sql.Format("insert into storage_name values (\"%s\",\"%s\")",storage_id,storage_name);
							if(mysql_query(&myCont,sql)!= 0)
							{
								const char *error = mysql_error(&myCont);
								CString str;
...
void Dialog_Storage_In::OnButtonModify() 
{
	m_bModify = 1;
	OnButtonQuery();	
}

void Dialog_Storage_In::OnColumnclickListTotal(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
HBRUSH Dialog_Storage_In::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor) 
{
...
void Dialog_Storage_In::OnButtonQuery() 
{
	m_bModify = 0;
	ResetStorageIn();
	GetConfig();			
	...
	//m_listStorageIn.DeleteAllItems();
	//UpdateData();
	CString csSql;

	csSql.Format("select time_make,time_payment,digest,provider,operator,department,storage  from storage_in_baseinfo where storage_in_id=\"%s\" " ,m_StorageInID); 
    ...
    if(mysql_real_connect(&myCont,g_MysqlConnect.host,g_MysqlConnect.user,g_MysqlConnect.pswd,g_MysqlConnect.table,g_MysqlConnect.port,NULL,0))
    {
        ...
        if(!res)
        {
		    ...
		    if(result)
            {
				...
				csSql.Format("select storage_id,storage_name,SCB,item_name,item_number,item_unit,item_price,item_money,item_digest from storage_in_item where storage_in_id=\"%s\" " ,m_StorageInID); 
				res=mysql_query(&myCont,csSql);//查询
				if(!res)
				{
