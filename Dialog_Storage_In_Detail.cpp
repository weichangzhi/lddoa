...
void Dialog_Storage_In_Detail::OnOK() 
{
	...
	{
	case 0://部门
		csSql.Format("select time_make,storage_in_id,time_payment,digest,provider,operator,department,storage  from storage_in_baseinfo where department=\"%s\" and time_make<=\"%s\" and time_make>=\"%s\" " ,m_strSubWay,endtime,starttime);
		break;
	case 1://仓库
		csSql.Format("select time_make,storage_in_id,time_payment,digest,provider,operator,department,storage  from storage_in_baseinfo where storage=\"%s\" and time_make<=\"%s\" and time_make>=\"%s\" " ,m_strSubWay,endtime,starttime);
		break;
	case 2://取货人
		csSql.Format("select time_make,storage_in_id,time_payment,digest,provider,operator,department,storage  from storage_in_baseinfo where provider=\"%s\" and time_make<=\"%s\" and time_make>=\"%s\" " ,m_strSubWay,endtime,starttime);
		break;
	case 3://经办人
		csSql.Format("select time_make,storage_in_id,time_payment,digest,provider,operator,department,storage  from storage_in_baseinfo where operator=\"%s\" and time_make<=\"%s\" and time_make>=\"%s\" " ,m_strSubWay,endtime,starttime);
		break;
	case 4://全部
...
void Dialog_Storage_In_Detail::OnSelchangeComboWay() 
{
	CString strpathini="";
	::GetCurrentDirectory(_MAX_PATH,strpathini.GetBuffer(_MAX_PATH));
	strpathini.ReleaseBuffer();
